...
public class ForceSimulatedMover : KinematicMover
{
    ...
    private Vector3 fromPointToHere = new Vector3();    // Points from the point of blowback to the position of this mover
    ...
    public void ApplyForce(Vector3 direction, float speed, float time = DEFAULT_BLOWBACK_TIME)
    {
    ...
    public void ApplyForceFromPoint(Vector3 point, float speed, float time = DEFAULT_BLOWBACK_TIME)
    {
        fromPointToHere = transform.position - point;
    ...
    public override void MoveTowards(Vector3 velocity)
    {
        if (!_isForced)
        {
    ...
    public override void MoveTowardsFrom(Vector3 origin, Vector3 direction, float speed)
    {
        if (!_isForced)
        {
    ...
    public void OverrideMoveTowards(Vector3 direction, float speed)
    {
        _isForced.Deactivate();
        base.MoveTowards(direction, speed);
    }
    public void OverrideMoveTowards(Vector3 velocity)
    {
        _isForced.Deactivate();
        base.MoveTowards(velocity);
    }
    ...
    public void OverrideMoveTowardsFrom(Vector3 origin, Vector3 direction, float speed)
    {
        _isForced.Deactivate();
        base.MoveTowardsFrom(origin, direction, speed);
    }
